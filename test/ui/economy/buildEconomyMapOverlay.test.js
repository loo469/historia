...
test('buildEconomyMapOverlay exposes city resource and logistics map layers', () => {
  ...
  assert.deepEqual(overlay.layers.logistics.features, [
    {
      ...
      recoveryPlanner: {
        id: 'recovery-planner:route-coast',
        targetType: 'route',
        targetId: 'route-coast',
        basedOnReplayId: 'replay:route-coast',
        summary: 'Coast Road (land): Prioriser convoi/stock recommandé après goulet grain.',
        dataCompleteness: 'complete',
        recommendedOptionId: 'recovery:route-coast:prioritize-stock',
        recommendationReason: 'Action immédiate issue du stress et de la priorité déjà visibles.',
        options: [
          {
            id: 'recovery:route-coast:prioritize-stock',
            ...
            reDisruptionRisk: 'medium',
            impact: 'Protège immédiatement grain avec les stocks et convois visibles.',
            impactScore: 8,
            affectedCityIds: ['city-harbor', 'city-mill'],
            affectedResourceIds: ['grain'],
          ...
          {
            id: 'recovery:route-coast:repair',
            action: 'repair',
            label: 'Réparer la route',
            badge: 'plan:réparer',
            relativeCost: 'élevé',
            costScore: 3,
            estimatedDelayTurns: 2,
            reDisruptionRisk: 'medium',
            impact: 'Restaure la capacité de Coast Road (land) pour 2 villes dépendantes.',
            impactScore: 10,
            affectedCityIds: ['city-harbor', 'city-mill'],
            affectedResourceIds: ['grain'],
          ...
          {
            id: 'recovery:route-coast:reroute',
            action: 'reroute',
            ...
            reDisruptionRisk: 'low',
            impact: 'Contourne grain pour garder un flux minimal vers 2 villes liées.',
            impactScore: 4,
            affectedCityIds: ['city-harbor', 'city-mill'],
            affectedResourceIds: ['grain'],
      ...
      recoveryCheckpoints: {
        id: 'recovery-checkpoints:route-coast',
        targetType: 'route',
        targetId: 'route-coast',
        linkedPlannerId: 'recovery-planner:route-coast',
        linkedOptionId: 'recovery:route-coast:prioritize-stock',
        status: 'stagnating',
        summary: 'Coast Road (land): checkpoints liés à Prioriser convoi/stock.',
        blockingSignals: ['surveiller grain'],
        dataCompleteness: 'complete',
        checkpoints: [
          {
            id: 'recovery-planner:route-coast:repair-started',
            stage: 'repair-started',
            label: 'Réparation lancée',
            status: 'complete',
            expectedTurn: 0,
            linkedOptionId: 'recovery:route-coast:prioritize-stock',
            signal: 'priorité engagée',
            detail: 'Prioriser convoi/stock: coût moyen, délai 0 tour(s).',
          },
          {
            id: 'recovery-planner:route-coast:partial-capacity',
            stage: 'partial-capacity',
            label: 'Capacité partielle',
            status: 'in-progress',
            expectedTurn: 1,
            linkedOptionId: 'recovery:route-coast:prioritize-stock',
            signal: 'capacité partielle',
            detail: '1/6 capacité restante visible après replay.',
          },
          {
            id: 'recovery-planner:route-coast:residual-risk',
            stage: 'residual-risk',
            label: 'Risque résiduel',
            status: 'watch',
            expectedTurn: 1,
            linkedOptionId: 'recovery:route-coast:prioritize-stock',
            signal: 'surveiller grain',
            detail: 'Risque de re-perturbation medium; revoir si nouveau goulot.',
          },
          {
            id: 'recovery-planner:route-coast:stable-return',
            stage: 'stable-return',
            label: 'Retour stable',
            status: 'pending',
            expectedTurn: 1,
            linkedOptionId: 'recovery:route-coast:prioritize-stock',
            signal: 'à confirmer',
            detail: 'Coast Road (land) doit être révisée si le replay reste recovering.',
          },
        ],
      },
      style: {
  ...
  assert.deepEqual(overlay.layers.logistics.recoveryPlannerLegend.map((entry) => entry.key), [
    'repair',
    'reroute',
    'prioritize-stock',
  ]);
  assert.deepEqual(overlay.layers.logistics.recoveryCheckpointLegend.map((entry) => entry.key), [
    'repair-started',
    'partial-capacity',
    'residual-risk',
    'stable-return',
