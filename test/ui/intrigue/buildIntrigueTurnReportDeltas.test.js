        ...
        returnFromBackupCondition: {
          ...
          backupLadderSummary: {
            ...
            stabilizationReason: {
              state: 'secure-dependency-before-return',
              recommended: true,
              label: 'Pourquoi sécuriser la dépendance',
              summary: 'risque sabotage: dépendance visible à sécuriser avant retour · exposition ajoutée: le retour direct peut raviver l’exposition visible · risque de skip: exposition',
              drivers: [
                'risque sabotage: dépendance visible à sécuriser avant retour',
                'exposition ajoutée: le retour direct peut raviver l’exposition visible',
                'risque de skip: exposition',
              ],
              fallback: false,
            },
            returnReadiness: {
              ...
              summary: 'Sécuriser la dépendance reste nécessaire avant de revenir vers Défensif.',
              nextStep: 'risque sabotage: dépendance visible à sécuriser avant retour · exposition ajoutée: le retour direct peut raviver l’exposition visible · risque de skip: exposition',
              postReturnExitCondition: {
                state: 'watch-after-return',
                watchRequired: true,
                label: 'Retour sous surveillance',
                condition: 'garder un œil après retour tant que exposition reste visible',
                reason: 'sécuriser la dépendance visible avant de restaurer le suivi initial: Retour possible mais temporaire: confirmer le signal avant de quitter le backup.',
                fallback: false,
              },
              fallback: false,
            },
      ...
      returnFromBackupCondition: {
        ...
        backupLadderSummary: {
          ...
          stabilizationReason: {
            state: 'hold-backup-before-return',
            recommended: true,
            label: 'Pourquoi maintenir la sauvegarde',
            summary: 'présence: garder le backup jusqu’à une fenêtre de retour fraîche · risque de skip: option qui se dégrade',
            drivers: [
              'présence: garder le backup jusqu’à une fenêtre de retour fraîche',
              'risque de skip: option qui se dégrade',
            ],
            fallback: false,
          },
          returnReadiness: {
            ...
            ready: false,
            label: 'Stabilisation encore requise',
            summary: 'Maintenir la sauvegarde reste nécessaire avant de revenir vers Offensif.',
            nextStep: 'présence: garder le backup jusqu’à une fenêtre de retour fraîche · risque de skip: option qui se dégrade',
            postReturnExitCondition: {
              state: 'watch-after-return',
              watchRequired: true,
              label: 'Retour sous surveillance',
              condition: 'garder un œil après retour tant que option qui se dégrade reste visible',
              reason: 'maintenir la sauvegarde jusqu’à une fenêtre de retour fraîche: Le retour risque de recréer un backup immédiatement après.',
              fallback: false,
            },
            fallback: false,
          },
    ...
    returnFromBackupCondition: {
      ...
      backupLadderSummary: {
        ...
        returnReadiness: {
          ...
          nextStep: 'Aucune stabilisation sûre n’est lisible avant retour avec les signaux visibles actuels.',
          fallback: true,
        },
        fallback: true,
      },
...
test('buildIntrigueTurnReportDeltas marks stable return when backup can safely hand back to the initial follow-up', () => {
  ...
  assert.deepEqual(report.minimumVerificationPrompt.safestMinimalVerification.returnFromBackupCondition.backupLadderSummary, {
    ...
    returnReadiness: {
      ...
      summary: 'Sortie prête vers Défensif: la durabilité visible est stable.',
      nextStep: 'revenir au suivi initial si la confiance visible se stabilise sans nouveau signal contraire',
      postReturnExitCondition: {
        state: 'backup-closed',
        watchRequired: false,
        ...
        reason: 'Le suivi initial peut tenir sans forcer un nouveau backup immédiat.',
        fallback: false,
      },
      fallback: false,
    },
