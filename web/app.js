...
function buildFollowUpClimatePayoffRecommendation(climateRiskReboundAfterTopPayoff, decisionDebtRanking, nextClimateCommitment) {
  if (!climateRiskReboundAfterTopPayoff) {
    return null;
  }

  const followUpDebt = decisionDebtRanking?.items?.find((item) => item.key === climateRiskReboundAfterTopPayoff.secondaryDebtKey) ?? null;

  if (!climateRiskReboundAfterTopPayoff.thresholdExceeded || !followUpDebt) {
    return {
      state: 'no-safe-follow-up',
      followUpDebtKey: null,
      followUpDebtLabel: null,
      reboundState: climateRiskReboundAfterTopPayoff.state,
      nextAction: 'Aucun paiement de suivi sûr ou utile avec les données actuelles.',
      reason: 'Le rebond reste sous le seuil ou la dette secondaire n’est pas assez fiable pour engager un paiement.',
      summary: 'Aucun paiement de suivi recommandé: surveiller la prochaine fenêtre.',
    };
  }

  const safeToPay = followUpDebt.key !== 'replace-commitment' && followUpDebt.estimatedDownstreamDebt !== 'haute';
  const state = safeToPay ? 'recommended' : 'no-safe-follow-up';
  const nextAction = safeToPay
    ? (nextClimateCommitment?.action ?? `Payer ensuite ${followUpDebt.label}.`)
    : 'Aucun paiement de suivi sûr ou utile avec les données actuelles.';
  const reason = safeToPay
    ? `${followUpDebt.label} réduit le rebond ${climateRiskReboundAfterTopPayoff.state} en traitant ${climateRiskReboundAfterTopPayoff.secondaryDebtCause}.`
    : `${followUpDebt.label} explique le rebond, mais le paiement reste trop fragile avec les signaux actuels.`;

  return {
    state,
    followUpDebtKey: followUpDebt.key,
    followUpDebtLabel: followUpDebt.label,
    reboundState: climateRiskReboundAfterTopPayoff.state,
    nextAction,
    reason,
    summary: state === 'recommended'
      ? `Paiement de suivi recommandé: ${followUpDebt.label} pour limiter ${climateRiskReboundAfterTopPayoff.state}.`
      : 'Aucun paiement de suivi sûr ou utile avec les données actuelles.',
  };
}
...
function buildSecondaryClimateDebtNearWindowReopenEffect(secondaryClimateDebtWhenReserveCostsWindow, climateReserveNearTermWindowCostWarning, climateRiskReboundAfterTopPayoff, decisionWindow) {
  if (!secondaryClimateDebtWhenReserveCostsWindow) {
    return null;
  }

  const constraintText = `${secondaryClimateDebtWhenReserveCostsWindow.visibleConstraint ?? ''} ${climateReserveNearTermWindowCostWarning?.visibleConstraint ?? ''} ${secondaryClimateDebtWhenReserveCostsWindow.summary ?? ''}`.toLowerCase();
  const visibleConstraint = secondaryClimateDebtWhenReserveCostsWindow.reserveStillBest
    ? 'réserve restante'
  ...
  const urgentDebt = secondaryClimateDebtWhenReserveCostsWindow.state === 'dette secondaire urgente';
  const watchedDebt = secondaryClimateDebtWhenReserveCostsWindow.state === 'dette à surveiller';
  const reboundContained = climateRiskReboundAfterTopPayoff?.state !== 'rebond fort';
  const state = urgentDebt && reboundContained && !hasTimingConflict
    ? 'fenêtre rouverte'
    : (urgentDebt || watchedDebt) && !secondaryClimateDebtWhenReserveCostsWindow.reserveStillBest
      ? 'fenêtre stabilisée mais fragile'
      : 'paiement insuffisant';
  const immediateGesture = state === 'fenêtre rouverte'
    ? `payer ${secondaryClimateDebtWhenReserveCostsWindow.secondaryDebtLabel} maintenant`
    : state === 'fenêtre stabilisée mais fragile'
      ? `payer ${secondaryClimateDebtWhenReserveCostsWindow.secondaryDebtLabel} puis garder une réserve courte`
      : 'aucun geste immédiat';
  const summary = state === 'paiement insuffisant'
    ? `Paiement insuffisant: la décision reste neutre; contrainte ${visibleConstraint}.`
    : `${state}: ${secondaryClimateDebtWhenReserveCostsWindow.secondaryDebtLabel} agit sur ${visibleConstraint}; geste: ${immediateGesture}.`;

  return {
    state,
    secondaryDebtKey: secondaryClimateDebtWhenReserveCostsWindow.secondaryDebtKey,
    secondaryDebtLabel: secondaryClimateDebtWhenReserveCostsWindow.secondaryDebtLabel,
    visibleConstraint,
    immediateGesture,
    changesDecision: state !== 'paiement insuffisant',
    reserveStillNeeded: state === 'fenêtre stabilisée mais fragile' || visibleConstraint === 'réserve restante',
    summary,
  };
}
...
function buildMinimalConsolidationNextClimateWindowPrevention(minimalConsolidationAfterFragileClimateReservePayment, residualReserveRiskAfterSecondaryClimateDebtPayment, decisionWindow) {
  if (!minimalConsolidationAfterFragileClimateReservePayment) {
    return null;
  }

  const constraintText = `${minimalConsolidationAfterFragileClimateReservePayment.visibleConstraint ?? ''} ${residualReserveRiskAfterSecondaryClimateDebtPayment?.visibleConstraint ?? ''} ${minimalConsolidationAfterFragileClimateReservePayment.summary ?? ''}`.toLowerCase();
  const visibleConstraint = constraintText.includes('cascade')
    ? 'cascade voisine'
    : constraintText.includes('conflit') || constraintText.includes('timing')
      ? 'conflit de timing'
      : constraintText.includes('saison')
        ...
        : constraintText.includes('pression régionale') || constraintText.includes('deadline')
          ...
          : constraintText.includes('dette')
            ? 'dette secondaire'
            : 'réserve restante';
  const hasWindowConflict = (decisionWindow?.conflicts?.length ?? 0) > 0;
  const state = minimalConsolidationAfterFragileClimateReservePayment.state === 'consolidation inutile'
    ? 'fenêtre proche empêchée'
    : minimalConsolidationAfterFragileClimateReservePayment.state === 'consolidation légère' && !hasWindowConflict
      ? 'fenêtre contenue'
      : 'complément requis';
  const smallestUsefulComplement = state === 'complément requis'
    ? 'ajouter une réserve anti-cascade courte'
    : state === 'fenêtre contenue'
      ? 'garder la consolidation légère jusqu’au prochain tour'
      : 'aucun complément utile';
  const summary = state === 'fenêtre proche empêchée'
    ? `Fenêtre proche empêchée: consolidation minimale suffisante; contrainte ${visibleConstraint}.`
    : `${state}: ${visibleConstraint}; plus petit complément: ${smallestUsefulComplement}.`;

  return {
    state,
    visibleConstraint,
    smallestUsefulComplement,
    changesNextStep: state === 'complément requis',
    sourceConsolidation: minimalConsolidationAfterFragileClimateReservePayment.state,
    sourceReserveRisk: residualReserveRiskAfterSecondaryClimateDebtPayment?.state ?? null,
    summary,
  };
}
...
function buildClimateRefreshReserveSpendingDownside(climateConsolidationRefreshVsReserveDecision, minimalClimateConsolidationProtectionDuration, minimalConsolidationNextClimateWindowPrevention) {
  if (!climateConsolidationRefreshVsReserveDecision) {
    return null;
  }

  const constraintText = `${climateConsolidationRefreshVsReserveDecision.refreshConstraint ?? ''} ${minimalClimateConsolidationProtectionDuration?.shorteningConstraint ?? ''} ${minimalConsolidationNextClimateWindowPrevention?.summary ?? ''}`.toLowerCase();
  const visibleDownside = climateConsolidationRefreshVsReserveDecision.state === 'réserve à conserver'
    ? 'fenêtre suivante affaiblie par dépense prématurée'
    : constraintText.includes('anomalie') || constraintText.includes('voisine') || constraintText.includes('cascade')
      ? 'cascade voisine moins couverte'
      : constraintText.includes('saison') || constraintText.includes('pression')
        ? 'pression saisonnière moins amortie'
        : constraintText.includes('dette')
          ? 'dette secondaire moins couverte'
          : 'réserve basse moins disponible';
  const state = climateConsolidationRefreshVsReserveDecision.state === 'réserve à conserver'
    ? 'dépense prématurée'
    : climateConsolidationRefreshVsReserveDecision.state === 'rafraîchissement urgent avant expiration'
      ? 'dépense opportune'
      : minimalClimateConsolidationProtectionDuration?.state === 'protection jusqu’à la prochaine fenêtre'
        ? 'dépense opportune'
        : 'dépense à temporiser';
  const timingMessage = state === 'dépense opportune'
    ? 'refresh utile: il protège la prochaine fenêtre au bon moment'
    : state === 'dépense prématurée'
      ? `downside climat: ${visibleDownside}`
      : `temporiser: ${visibleDownside} si la réserve part trop tôt`;
  const summary = `${state}: ${timingMessage}.`;

  return {
    state,
    visibleDownside,
    timingMessage,
    sourceRefreshDecision: climateConsolidationRefreshVsReserveDecision.state,
    sourceProtectionDuration: minimalClimateConsolidationProtectionDuration?.state ?? null,
    weakensNextClimateWindow: state !== 'dépense opportune',
    summary,
  };
}
...
function buildAtlasClimateCheapestSafeRecoveryCommitment(recoveryProjectionView) {
  if (!recoveryProjectionView || recoveryProjectionView.state === 'neutral' || !recoveryProjectionView.projection) {
    return {
      ...
      climateRefreshReserveSpendingDownside: null,
  ...
  const followUpClimatePayoffRecommendation = buildFollowUpClimatePayoffRecommendation(
  ...
  const secondaryClimateDebtNearWindowReopenEffect = buildSecondaryClimateDebtNearWindowReopenEffect(
  ...
  const minimalConsolidationNextClimateWindowPrevention = buildMinimalConsolidationNextClimateWindowPrevention(
  ...
  const climateRefreshReserveSpendingDownside = buildClimateRefreshReserveSpendingDownside(
  ...
  return {
    ...
    minimalClimateConsolidationProtectionDuration,
    climateConsolidationRefreshVsReserveDecision,
    climateRefreshReserveSpendingDownside,
...
function renderAtlasClimateCheapestSafeRecoveryCommitment(view) {
  ...
  return `
    <aside class="map-world-climate-cheapest-commitment map-world-climate-cheapest-commitment--${view.state}" aria-label="Engagement recovery climat minimal sûr">
      ...
      ${view.climateRefreshReserveSpendingDownside ? `<small><b>Coût dépense réserve refresh</b> · ${view.climateRefreshReserveSpendingDownside.summary}</small>` : ''}
