...
- côté UI, `buildCultureLayerPanel` assemble un focus lisible par région et culture pour explorer les marqueurs, découvertes, progression des recherches et événements historiques depuis la carte, avec des filtres internes simples et une comparaison régionale compacte pour isoler et comparer rapidement plusieurs cultures visibles
- côté carte, `buildCultureMapOverlay` transforme cultures, recherches et événements historiques en marqueurs régionaux stables avec score d'influence, tier visuel, highlights, `eventPopups`, `regionalDiscoveryLinks` et un contrat visuel enrichi (`accent`, `labelTone`, `surface`, `glow`) pour mieux distinguer les zones culturelles, ouvrir les événements depuis la carte et relier les découvertes aux régions concernées
...
- dans `web/app.js`, l'overlay économie expose un HUD logistique sombre avec panneaux en verre dépoli, bande KPI dense, focus ville sélectionnée, glyphes de ressources, badges de stock par ville, manifeste des cargaisons par route, halos de tension, paquets de flux, pictogrammes de mode de transport, ticks de risque et légende compacte pour lire routes, hubs, capacités et pénuries dans le style Pax Historia sans surcharger la carte
- dans `web/app.js`, la démo web peut afficher un overlay intrigue dédié avec filtres séparés pour présence, alertes et sabotage, liens de réseau, foyers cliquables, heatmap de sabotage, panneau d’alerte dédié, lecture locale du risque pour la province sélectionnée, détails d’opérations actives au clic, journal compact des incidents de sécurité, et codes visuels distincts pour cellules dormantes, exposées et compromises, afin de rendre la présence clandestine plus lisible sans surcharger la carte
