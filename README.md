...
- dans `web/app.js`, la démo web peut afficher un overlay intrigue dédié avec liens de réseau, foyers cliquables, heatmap de sabotage, panneau d’alerte dédié, lecture locale du risque pour la province sélectionnée, détails d’opérations actives au clic, journal compact des incidents de sécurité, et codes visuels distincts pour cellules dormantes, exposées et compromises, afin de rendre la présence clandestine plus lisible sans surcharger la carte
